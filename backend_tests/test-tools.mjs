...
async function testAgentTools() {
    ...
    const tour = new GuidedTour({ startUrl: 'https://www.youtube.com/?app=desktop&hl=tr' });
    await tour.open();
    ...
    const analysis = await analyzeFrame(base64Image, 'Burada hangi videoları görüyorsun?');
