...
- **Knowledge manager**
  ...
  - **Site Bilgisi (Konu Ağacı + Site Yapısı)** — `KnowledgeGaps.jsx`'e
    üçüncü sekme olarak eklendi (bkz. `md/backend/phase1_rag_ingestion.md`'in
    "Site Bilgisi" girişi — arka planda ne çalıştığının tam açıklaması
    orada). `SiteTopicsTab`:
    - **Konu Ağacı** — `GET /knowledge/:productId/topics`'in düz listesini
      (`parentTopicId`) client-side ağaca çeviren `TopicNode`, recursive,
      accordion. Her node: başlık, `autoGenerated` rozeti (siteye özgü,
      otomatik açılmış konular için), boşsa "içerik bulunamadı" notu,
      genişletilince markdown gövde + `sourcePages` chip'leri (yukarıdaki
      `?source=` deep-link mekanizmasının aynısıyla kaynağa gidiyor) +
      inline "Düzenle" (textarea, `PATCH /knowledge/topics/:id` ile kaydeder
      — kaydetme sunucu tarafında topic'in chunk'larını tam re-embed eder).
    - **Site Yapısı** — sadece istenince (`showSitemap` state, `enabled:
      showSitemap`) yüklenen, salt-okunur bir liste: `GET
      /knowledge/:productId/sitemap`'in döndürdüğü her sayfa için URL,
      `parentUrl` ("↳ ..." satırı), link/buton sayısı, ve (varsa)
      `components` sayıları (`N sayfa-içi öğe, M başlık` — tıklanınca başlık/
      buton/bölüm etiketlerinin salt-okunur listesini açıyor).
    - Ayrı bir API çağrısı/mutasyonu gerektirmeyen basit bir sekme —
      `gapReportPdf.js` gibi ağır bir dinamik-import'a ihtiyaç duymadı,
      `KnowledgeGaps` chunk'ına doğrudan eklendi (üretim build'inde
      ~19KB, kabul edilebilir).
  - **Knowledge sayfasında görünürlük** — kullanıcı geri bildirimi: otomatik
    üretilen konu dokümanları sadece "Bilgi boşlukları" sayfasının 3.
    sekmesinde görünüyordu, Knowledge ana sayfasında (kaynak listesinde) hiç
    izi yoktu. `Knowledge.jsx` artık her url/api kaynağının altında (zip-child
    genişletme desenindeki aynı `isGroup`/`expanded` mekanizmasıyla, ama
    `KnowledgeTopic`'i sanal bir "çocuk" olarak) o kaynağa katkı sağlamış,
    içeriği dolu konu dokümanlarını (`TopicPreviewRow`, "Otomatik oluşturuldu"
    rozetli) listeliyor. Tıklanınca açılan `TopicPreviewModal` SALT-OKUNUR —
    düzenleme bilinçli olarak "Bilgi boşlukları" sayfasında bırakıldı (modal
    sadece oraya, ilgili sekmeye giden bir "Düzenle" linki taşıyor) —
    kullanıcı Knowledge sayfasının karmaşıklaşmasını istemedi.
