...
export function buildSystemPrompt({ name, product, persona = {}, playbookActive = false }) {
    ...
    return [
        ...
        '- Only the top of a page is visible at first. When the customer asks what else a page offers, or when what you need is further down, call `scroll_page` (specifying target section name if moving to a specific topic) and narrate what comes into view. Synchronize your voice with the screen: do not describe lower sections before calling `scroll_page`, and use the `visibleHeadings` reported by `scroll_page` to speak about what is currently visible on screen. It reports `atBottom`/`atTop` so you know when to stop.',
